...
// This will be replaced at build-time by the shell script:
const API_URL = "51:/api/aiQuote";
...
function deriveTimeline(mid: number) {
  if (mid < 4000) return { weeks: "1–2 weeks", crew: "2–3", materials: "Light", risk: "Low" };
  if (mid < 8000) return { weeks: "2–5 weeks", crew: "3–4", materials: "Moderate", risk: "Medium" };
  if (mid < 15000) return { weeks: "3–6 weeks", crew: "4–5", materials: "Heavy", risk: "Medium" };
  return { weeks: "6–10 weeks", crew: "5–7", materials: "Extensive", risk: "Medium" };
}
...
export default function AiCinematicEstimator() {
  ...
  /* Typewriter demo loop (stops when external prompt arrives) */
  useEffect(() => {
    if (!running) return;
  ...
  /* Listen for external rotator prompts */
  useEffect(() => {
  ...
  async function preview(q: string) {
    try {
      ...
      const body = { service: q, details: q }; // compatible with current preview API
      const r = await fetch(API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
      const data: any = await parseMaybeJson(r);
      const est: Estimate | undefined =
        data?.estimate || data?.data?.estimate || (Array.isArray(data?.estimates) ? data.estimates[0] : undefined);

      if (!est || typeof est.low !== "number" || typeof est.high !== "number") {
        throw new Error("No estimate");
      }
      const m = Math.round((est.low + est.high) / 2);
      setLow(est.low); setMid(m); setHigh(est.high);
    } catch (e: any) {
      setErr(e?.message || "Preview failed");
      // keep last numbers; UI still looks good
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="rounded-2xl border border-gray-200 bg-white/80 backdrop-blur-sm p-4 sm:p-5 shadow-sm relative overflow-hidden">
      ...
      <div className="mt-3 flex items-center gap-2">
        <div className="relative grow">
          <input readOnly value={typed} className="w-full rounded-lg border border-gray-300 bg-white pl-3 pr-28 py-2 text-sm" />
          <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-xs">
            <span className="ai-caret">|</span>
          </span>
        </div>
        <button onClick={() => preview(typed || phrase)} className="px-3 py-2 rounded-lg border text-sm hover:bg-gray-50">
          Preview
        </button>
      </div>

      <div className="mt-3 grid grid-cols-3 gap-2 sm:gap-3">
        <PriceCard label="Low" value={useTween(low)} highlight={loading} />
        <PriceCard label="Estimate" value={useTween(Math.round((low+high)/2))} highlight={loading} />
        <PriceCard label="High" value={useTween(high)} highlight={loading} />
      </div>
      ...
      <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3">
        <div className="flex items-center justify-between text-[12px] text-slate-600">
          ...
          <div className="text-slate-500">Confidence: <b>{(mid<4000)?"High":(mid<12000)?"Med–High":"Medium"}</b></div>
        </div>
        ...
        <div className="mt-2 grid grid-cols-3 gap-2 text-[12px] text-slate-600">
          <div className="flex items-center gap-1.5"><span aria-hidden>🧰</span> Crew: <b>{deriveTimeline(mid).crew}</b></div>
          <div className="flex items-center gap-1.5"><span aria-hidden>🧱</span> Materials: <b>{deriveTimeline(mid).materials}</b></div>
          <div className="flex items-center gap-1.5"><span aria-hidden>⏱️</span> Timeline: <b>{deriveTimeline(mid).weeks}</b></div>
        </div>
      </div>

      <div className="mt-2 text-[11px] sm:text-xs text-gray-500">
        Estimated timeline: <b>{deriveTimeline(mid).weeks}</b> · Data from 2,300+ real Kent projects.
