...
/**
 * When a prompt finishes typing, this component dispatches:
 *   window.dispatchEvent(new CustomEvent("brixel:ai/prompt", { detail: { prompt } }));
 */
type Props = {
  prompts?: string[];
  ...
  useEffect(() => {
    ...
    if (phase === "typing") {
      if (typed.length < current.length) {
        ...
        setPhase("holding");
        // broadcast exactly once per completion
        if (current && lastBroadcast.current !== current) {
          lastBroadcast.current = current;
          try {
            window.dispatchEvent(new CustomEvent("brixel:ai/prompt", { detail: { prompt: current } }));
          } catch {}
        }
