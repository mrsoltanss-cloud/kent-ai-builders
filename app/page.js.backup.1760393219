      ...
      <header className="sticky top-0 z-40 w-full border-b bg-white/90 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-[15px]">
            <Link href="/my/portal" className="hover:opacity-80">
              Homeowner
            </Link>
...
<div className="relative mt-6 sm:mt-8">
            ...
            <button aria-label="scroll right" onClick={() => scrollBy(320)} className="hidden md:grid place-content-center absolute right-0 top-1/2 -translate-y-1/2 bg-white border rounded-full w-10 h-10 shadow">›</button>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#0f1b24] text-white">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 py-8 sm:py-10 grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          <div>
            ...
            <p className="text-white/80 text-sm">✉️ hello@brixel.uk</p>
          </div>
          <div>
