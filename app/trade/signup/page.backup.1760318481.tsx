          ...
          <Magnetic strength={0.08}>
            <Tilt max={12} className="rounded-3xl glass-card rim-gold px-4 sm:px-6 py-8 sm:p-12 text-center text-white">
              ...
              >
                ...
                <div className="mt-8">
                  <RippleButton
                    onClick={() => (window.location.href = "/api/auth/signin?provider=email&callbackUrl=/trade/onboarding")}
                    className="h-12 w-full rounded-xl bg-cyan-500 text-white hover:bg-cyan-600 transition-colors text-base font-semibold"
                  >
                    Continue with Email
                <p className="mt-3 text-slate-300/90 text-sm"><a href="/trade/onboarding" className="hover:underline">Explore onboarding</a></p>
                  </RippleButton>
                </div>

                <p className="text-slate-300 text-sm mt-6">
                  Already a member?{" "}
                  <a href="/api/auth/signin?provider=email&callbackUrl=/trade/onboarding"callbackUrl=/trade/onboarding"callbackUrl=/trade/onboarding" className="text-cyan-300 hover:underline">
                    Sign in
                  </a>
                </p>
